...
Quarkus provides a Jakarta Persistence identity provider, similar to the link:{quarkusio-guides}/security-jdbc[JDBC identity provider]. Jakarta Persistence is suitable for use with the link:{default-url}/basic-authentication/security-basic-authentication[Basic] and link:{default-url}/security-architecture/security-authentication-mechanisms#form-auth[Form-based] Quarkus Security mechanisms, which require username and password credentials.
...
For a practical example, see the link:{default-url}/getting-started-with-security/security-getting-started-tutorial[Getting started with Security using Basic authentication and Jakarta Persistence] tutorial.
...
* The `@UserDefinition` annotation must be present on a Jakarta Persistence entity, regardless of whether link:{quarkusio-guides}/hibernate-orm-panache[simplified Hibernate ORM with Panache] is used or not.
...
[TIP]
====
The link:{quarkusio-guides}/hibernate-orm#multitenancy[Hibernate Multitenancy] is supported, and you can store the user entity in a persistence unit with enabled multitenancy.
...
For more information about proactive authentication, see the Quarkus link:{default-url}/security-architecture/security-proactive-authentication[Proactive authentication] guide.
====
