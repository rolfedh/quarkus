...
For more information about OIDC Bearer token authentication, see the Quarkus link:{default-url}/openid-connect-authentication/security-oidc-bearer-token-authentication[OpenID Connect (OIDC) Bearer token authentication] guide.

If you want to protect web applications by using OIDC Authorization Code Flow authentication, see the link:{default-url}/openid-connect-authentication/security-oidc-code-flow-authentication[OpenID Connect authorization code flow mechanism for protecting web applications] guide.
...
For more information, see the xref:keycloak-dev-mode_{context}[Run the application in dev mode] section.
...
For more information, see the Quarkus link:{default-url}/openid-connect-authentication/security-oidc-configuration-properties-reference[OpenID Connect (OIDC) configuration properties] guide.
...
For more information, see the link:{quarkusio-guides}/security-keycloak-admin-client[Quarkus Keycloak Admin Client] guide.

====
endif::no-quarkus-keycloak-admin-client[]


[id="keycloak-dev-mode_{context}"]
...
. Open a link:{quarkusio-guides}/dev-ui[Dev UI], which you can find at http://localhost:8080/q/dev-ui[/q/dev-ui].
...
For information about testing your application in dev mode, see the preceding xref:keycloak-dev-mode_{context}[Run the application in dev mode] section.
...
For information about writing integration tests that depend on `Dev Services for Keycloak`, see the link:{default-url}/openid-connect-authentication/security-oidc-bearer-token-authentication#integration-testing-keycloak-devservices[Dev Services for Keycloak] section of the "OpenID Connect (OIDC) Bearer token authentication" guide.
...
* link:{default-url}/openid-connect-authentication/security-oidc-bearer-token-authentication[OpenID Connect (OIDC) Bearer token authentication]
