...
Get started with Quarkus Security by securing your Quarkus application endpoints with the built-in Quarkus link:{default-url}/basic-authentication/security-basic-authentication[Basic authentication] and the Jakarta Persistence identity provider, enabling role-based access control.

The Jakarta Persistence `IdentityProvider` verifies and converts a link:{default-url}/basic-authentication/security-basic-authentication[Basic authentication] user name and password pair to a `SecurityIdentity` instance, which is used to authorize access requests, making your Quarkus application secure.

For more information about Jakarta Persistence, see the link:{default-url}/getting-started-with-security/security-jpa[Quarkus Security with Jakarta Persistence] guide.
...
[NOTE]
====
link:{quarkusio-guides}/hibernate-orm-panache[Hibernate ORM with Panache] is used to store your user identities, but you can also use link:{quarkusio-guides}/hibernate-orm[Hibernate ORM] with the `security-jpa` extension.

ifndef::no-quarkus-security-jpa-reactive[]
Both link:{quarkusio-guides}/hibernate-reactive[Hibernate Reactive] and link:{quarkusio-guides}/hibernate-reactive-panache[Hibernate Reactive with Panache] can be used with the `security-jpa-reactive` extension.
endif::no-quarkus-security-jpa-reactive[]
...
[NOTE]
====
Don’t forget to set up the Panache and PostgreSQL JDBC driver, please see link:{quarkusio-guides}/hibernate-orm-panache#setting-up-and-configuring-hibernate-orm-with-panache[Setting up and configuring Hibernate ORM with Panache] for more information.
====
ifndef::no-quarkus-security-jpa-reactive[]
[NOTE]
====
...
. Enable the built-in Quarkus link:{default-url}/basic-authentication/security-basic-authentication[Basic authentication] mechanism by setting the `quarkus.http.auth.basic` property to `true`:
+
`quarkus.http.auth.basic=true`
+
[NOTE]
====
When secure access is required, and no other authentication mechanisms are enabled, the built-in link:{default-url}/basic-authentication/security-basic-authentication[Basic authentication] of Quarkus is the fallback authentication mechanism.
...
Complete the integration testing of your application in JVM and native modes by using link:{quarkusio-guides}/dev-services#databases[Dev Services for PostgreSQL] before you run your application in production mode.
...
While developing your application, you can add and run tests individually by using the link:{quarkusio-guides}/continuous-testing[Continuous Testing] feature.
...
This was achieved by integrating the built-in link:{default-url}/basic-authentication/security-basic-authentication[Basic authentication] in Quarkus with the Jakarta Persistence identity provider.
...
* link:{default-url}/openid-connect-authentication/security-oidc-code-flow-authentication[OIDC code flow mechanism for protecting web applications]
...
* link:{default-url}/security-architecture/security-authentication-mechanisms#other-supported-authentication-mechanisms[Other supported authentication mechanisms]
* link:{default-url}/security-architecture/security-identity-providers[Identity providers]
* link:{default-url}/openid-connect-authentication/security-oidc-bearer-token-authentication[OIDC Bearer token authentication]
* link:{default-url}/openid-connect-authentication/security-oidc-code-flow-authentication[OIDC code flow mechanism for protecting web applications]
* link:{quarkusio-guides}/hibernate-orm-panache[Simplified Hibernate ORM with Panache]
* link:{quarkusio-guides}/hibernate-orm[Using Hibernate ORM and Jakarta Persistence]
