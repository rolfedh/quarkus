...
For more information about `Oidc Client` and `Token Propagation` support in Quarkus, see the link:{default-url}/openid-connect-authentication/security-openid-connect-client-reference[OpenID Connect (OIDC) and OAuth2 client and filters reference guide].

To protect your applications by using Bearer Token Authorization, see the link:{default-url}/openid-connect-authentication/security-oidc-bearer-token-authentication[OpenID Connect (OIDC) Bearer token authentication] guide.
...
=======
For more information, see the xref:keycloak-dev-mode_{context}[Running the application in dev mode] section.
>>>>>>> Stashed changes
...
link:{quarkusio-guides}/security-openid-connect-dev-services[Dev Services for Keycloak] launches a Keycloak container and imports `quarkus-realm.json`.

Open a link:{quarkusio-guides}/dev-ui[Dev UI] available at http://localhost:8080/q/dev-ui[/q/dev-ui] and click a `Provider: Keycloak` link in the *OpenID Connect Dev UI* card.
...
=======
For more information about testing your application in dev mode, see the preceding xref:keycloak-dev-mode_{context}[Running the application in dev mode] section.
>>>>>>> Stashed changes
...
* link:{default-url}/openid-connect-authentication/security-openid-connect-client-reference[OpenID Connect Client and Token Propagation Reference Guide]
