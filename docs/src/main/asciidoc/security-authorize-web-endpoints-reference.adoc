...
link:{quarkusio-guides}/security-customization#security-identity-customization[Security Identity Customization].
...
The `quarkus.http.root-path` configuration property changes the link:{quarkusio-guides}/http-reference#context-path[http endpoint context path].
...
CAUTION: If you plan to use standard security annotations on the IO thread, review the information in link:{default-url}/security-architecture/security-proactive-authentication[Proactive Authentication].

The `@RolesAllowed` annotation value supports link:{quarkusio-guides}/config-reference#property-expressions[property expressions] including default values and nested property expressions.
...
CAUTION: If you plan to use the `@PermissionsAllowed` on the IO thread, review the information in link:{default-url}/security-architecture/security-proactive-authentication[Proactive Authentication].

NOTE: `@PermissionsAllowed` is not repeatable on the class level due to a limitation with Quarkus interceptors.
For more information, see the link:{quarkusio-guides}/cdi-reference#repeatable-interceptor-bindings[Repeatable interceptor bindings] section of the Quarkus "CDI reference" guide.
...
automatically by the link:{quarkusio-guides}/security-openid-connect-dev-services[Dev Services for Keycloak] has an `admin` role.
...
In the following example, link:{quarkusio-guides}/security-customization#security-identity-customization[`SecurityIdentity` is customized] to add the same permission that was previously granted with the HTTP role-based policy.
...
CAUTION: Annotation-based permissions do not work with custom link:{quarkusio-guides}/security-customization#jaxrs-security-context[Jakarta REST SecurityContexts] because there are no permissions in `jakarta.ws.rs.core.SecurityContext`.
...
* link:{default-url}/security-architecture/security-authentication-mechanisms#other-supported-authentication-mechanisms[Authentication mechanisms in Quarkus]
...
* link:{default-url}/openid-connect-authentication/security-oidc-bearer-token-authentication#token-scopes-and-security-identity-permissions[OpenID Connect Bearer Token Scopes And SecurityIdentity Permissions]
