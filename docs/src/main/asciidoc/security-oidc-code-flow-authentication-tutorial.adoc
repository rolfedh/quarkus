...
For more information, see link:{default-url}/openid-connect-authentication/security-oidc-code-flow-authentication[OIDC code flow mechanism for protecting web applications].

To learn about how well-known social providers such as Apple, Facebook, GitHub, Google, Mastodon, Microsoft, Twitch, Twitter (X), and Spotify can be used with Quarkus OIDC, see link:{quarkusio-guides}/security-openid-connect-providers[Configuring well-known OpenID Connect providers].
See also, link:{default-url}/security-architecture/security-authentication-mechanisms#other-supported-authentication-mechanisms[Authentication mechanisms in Quarkus].

If you want to protect your service applications by using OIDC Bearer token authentication, see link:{default-url}/openid-connect-authentication/security-oidc-bearer-token-authentication[OIDC Bearer token authentication].
...
For more information, see the link:{default-url}/openid-connect-authentication/security-oidc-code-flow-authentication#access_id_and_access_tokens[Access ID and Access Tokens] section of the reference guide.
...
For more information, see link:{default-url}/authorization/security-authorize-web-endpoints-reference[Security Authorization Guide].
...
For example, you can access the Keycloak Admin console directly from the dev UI by clicking the `Keycloak Admin` link if you use link:{default-url}/openid-connect-authentication/security-oidc-code-flow-authentication#integration-testing-keycloak-devservices[Dev Services for Keycloak] in dev mode:

image::dev-ui-oidc-keycloak-card.png[alt=Dev UI OpenID Connect Card,role="center"]

For more information about writing the integration tests that depend on `Dev Services for Keycloak`, see the link:{default-url}/openid-connect-authentication/security-oidc-code-flow-authentication#integration-testing-keycloak-devservices[Dev Services for Keycloak] section.
...
After you have completed this tutorial, explore link:{default-url}/openid-connect-authentication/security-oidc-bearer-token-authentication[OIDC Bearer token authentication] and link:{default-url}/security-architecture/security-authentication-mechanisms[other authentication mechanisms].
...
* link:{quarkusio-guides}/security-oidc-auth0-tutorial[Protect Quarkus web application by using Auth0 OpenID Connect provider]
