...
JBoss Logging, integrated into Quarkus, offers a unified configuration for all xref:logging-apis[supported logging APIs] through a single configuration file that sets up all available extensions.
...
To send logs to a centralized tool such as Graylog, Logstash, or Fluentd, see the Quarkus link:{quarkusio-guides}/centralized-log-management[Centralized log management] guide.
...
See also link:{quarkusio-guides}/getting-started-testing#test-from-ide[Running `@QuarkusTest` from an IDE].
