...
For more information about consuming and using a reactive datasource, see the Quarkus link:{quarkusio-guides}/reactive-sql-clients[Reactive SQL clients] guide.

Additionally, refer to the Quarkus link:{quarkusio-guides}/hibernate-orm[Hibernate ORM] guide for information on consuming and using a JDBC datasource.
...
For more information, see the link:{quarkusio-guides}/config-reference#profiles[Profiles] section of the "Configuration reference" guide.

For more information about Dev Services, see link:{quarkusio-guides}/dev-services[Dev Services overview].

For more details and optional configurations, see link:{quarkusio-guides}/databases-dev-services[Dev Services for databases].
...
[NOTE]
====
The Hibernate ORM extension supports defining link:{quarkusio-guides}/hibernate-orm#multiple-persistence-units[persistence units] by using configuration properties.
...
Generally, this is the `db-kind` property, but you can also specify Dev Services properties to create named datasources according to the link:{quarkusio-guides}/databases-dev-services[Dev Services for Databases] guide.
...
For example see link:{quarkusio-guides}/hibernate-orm#persistence-unit-active[here for Hibernate ORM].
====
...
Setting `quarkus.datasource."pg".active=true` link:{quarkusio-guides}/config-reference#configuration-sources[at runtime]
...
[TIP]
====
link:{quarkusio-guides}/config-reference#custom-profiles[Custom configuration profiles] can help simplify such a setup.
...
link:{quarkusio-guides}/config-reference#multiple-profiles[setting `quarkus.profile`]:
...
[TIP]
====
It can also be useful to define a link:{quarkusio-guides}/cdi#ok-you-said-that-there-are-several-kinds-of-beans[CDI bean producer] redirecting to the currently active datasource,
like this:
...
If you are using the link:{quarkusio-guides}/telemetry-micrometer[`quarkus-micrometer`] or link:{quarkusio-guides}/smallrye-metrics[`quarkus-smallrye-metrics`] extension, `quarkus-agroal` can contribute some datasource-related metrics to the metric registry.
...
To use tracing with a datasource, you need to add the link:{quarkusio-guides}/opentelemetry[`quarkus-opentelemetry`] extension to your project.

You don't need to declare a different driver because you need tracing. If you use a JDBC driver, you need to follow the instructions in the OpenTelemetry extension link:{quarkusio-guides}/opentelemetry#jdbc[here].
...
To facilitate the storage of transaction logs in a database by using JDBC, see link:{quarkusio-guides}/transaction#jdbcstore[Configuring transaction logs to be stored in a datasource] section of the link:{quarkusio-guides}/transaction[Using transactions in Quarkus] guide.
...
The recommended approach is to use the real database you intend to use in production, especially when link:{quarkusio-guides}/databases-dev-services[Dev Services provide a zero-config database for testing], and running tests against a container is relatively quick and produces expected results on an actual environment.
