...
:context: security-oidc-bearer-token-authentication
...
If you need to authenticate and authorize users by using OIDC authorization code flow, see the Quarkus link:{default-url}/openid-connect-authentication/security-oidc-code-flow-authentication[OpenID Connect authorization code flow mechanism for protecting web applications] guide.
Also, if you use Keycloak and bearer tokens, see the Quarkus link:{quarkusio-guides}/security-keycloak-authorization[Using Keycloak to centralize authorization] guide.
...
* link:{default-url}/openid-connect-authentication/security-oidc-bearer-token-authentication-tutorial[Protect a web application by using OpenID Connect (OIDC) authorization code flow].

For information about how to support multiple tenants, see the Quarkus link:{default-url}/openid-connect-authentication/security-openid-connect-multitenancy[Using OpenID Connect Multi-Tenancy] guide.
...
For more information, see the link:{quarkusio-guides}/security-jwt#supported-injection-scopes[Supported injection scopes] section of the Quarkus "Using JWT RBAC" guide.
...
[id="token-claims-and-security-identity-roles"]
...
For more information, see the link:{quarkusio-guides}/security-customization#security-identity-customization[Security identity customization] section of the Quarkus "Security tips and tricks" guide.

You can also map `SecurityIdentity` roles created from token claims to deployment-specific roles by using the link:{default-url}/authorization/security-authorize-web-endpoints-reference#map-security-identity-roles[HTTP Security policy].

[id="token-scopes-and-security-identity-permissions"]
...
For more information about the `io.quarkus.security.PermissionsAllowed` annotation, see the link:{default-url}/authorization/security-authorize-web-endpoints-reference#permission-annotation[Permission annotation] section of the "Authorization of web endpoints" guide.
...
For information about bearer access token propagation to the downstream services, see the link:{default-url}/openid-connect-authentication/security-openid-connect-client-reference#token-propagation[Token propagation] section of the Quarkus "OpenID Connect (OIDC) and OAuth2 client and filters reference" guide.
...
For more information about supported authentication options, see the  link:{default-url}/openid-connect-authentication/security-oidc-code-flow-authentication#oidc-provider-client-authentication[OIDC provider client authentication] section in the Quarkus "OpenID Connect authorization code flow mechanism for protecting web applications" guide.
...
[NOTE]
====
If you have to test Quarkus OIDC service endpoints that require link:{quarkusio-guides}/security-keycloak-authorization[Keycloak authorization], follow the link:{quarkusio-guides}/security-keycloak-authorization#testing[Test Keycloak authorization] section.
====
...
[id="integration-testing-oidc-test-client"]
=== `OidcTestClient`
...
To start, add the same dependency, `quarkus-test-oidc-server`, as described in the xref:integration-testing-wiremock_{context}[WireMock] section.
...
The preferred approach for integration testing against Keycloak is link:{quarkusio-guides}/security-openid-connect-dev-services[Dev Services for Keycloak].
...
For more information about initializing and configuring Dev Services for Keycloak, see the link:{quarkusio-guides}/security-openid-connect-dev-services[Dev Services for Keycloak] guide.
...
[IMPORTANT]
====
Use xref:integration-testing-keycloak-devservices_{context}[Dev Services for Keycloak] instead of `KeycloakTestResourceLifecycleManager` for integration testing with Keycloak, unless you have specific requirements for using `KeycloakTestResourceLifecycleManager`.
====
...
[id="integration-testing-public-key"]
==== Local public key
...
To generate JWT tokens, copy `privateKey.pem` from the `integration-tests/oidc-tenancy` in the `main` Quarkus repository and use a test code similar to the one in the preceding xref:integration-testing-wiremock_{context}[WireMock] section.
...
For example, one situation when you must verify the token after the HTTP request has completed is when you are processing messages with link:{quarkusio-guides}/vertx#event-bus[Vert.x event bus].
...
the link:{default-url}/openid-connect-authentication/security-openid-connect-multitenancy[Using OpenID Connect Multi-Tenancy] guide.
...
[WARNING]
====
link:{default-url}/openid-connect-authentication/security-openid-connect-multitenancy#tenant-config-resolver[Dynamic tenant configuration resolution] is currently not supported.
...
For more information, see xref:security-oidc-code-flow-authentication#oidc-request-filters_{context}[OIDC request filters].
...
* link:{quarkusio-guides}/security-jwt-build[Sign and encrypt JWT tokens with SmallRye JWT Build]
...
* link:{quarkusio-guides}/security-keycloak-admin-client[Quarkus Keycloak Admin Client]
endif::no-quarkus-keycloak-admin-client[]
* link:{default-url}/openid-connect-authentication/security-openid-connect-multitenancy[Using OpenID Connect Multi-Tenancy]
