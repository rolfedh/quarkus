...
|Username and password |link:{default-url}/basic-authentication/security-basic-authentication[Basic], xref:form-auth[Form-based authentication]

|Bearer access token |link:{default-url}/openid-connect-authentication/security-oidc-bearer-token-authentication[OIDC Bearer token authentication], link:{quarkusio-guides}/security-jwt[JWT]
ifndef::no-quarkus-elytron-security-oauth2[]
, link:{quarkusio-guides}/security-oauth2[OAuth2]
endif::no-quarkus-elytron-security-oauth2[]

|Single sign-on (SSO) |link:{default-url}/openid-connect-authentication/security-oidc-code-flow-authentication[OIDC Code Flow], xref:form-auth[Form-based authentication]

|Client certificate |xref:mutual-tls[Mutual TLS authentication]

ifndef::no-webauthn-authentication[]
|WebAuthn |link:{quarkusio-guides}/security-webauthn[WebAuthn]
...
* link:{default-url}/basic-authentication/security-basic-authentication[Basic authentication]
** link:{default-url}/basic-authentication/security-basic-authentication-howto[Enable Basic authentication]
...
For more information, see the link:{quarkusio-guides}/http-reference#ssl[Supporting secure connections with SSL/TLS] section of the Quarkus "HTTP reference" guide.
...
For example, you can update the properties shown in the section which introduces xref:mutual-tls[Mutual TLS authentication] as follows:
...
You can always register `SecurityIdentityAugmentor` if the automatic xref:map-certificate-attributes-to-roles[Mapping certificate attributes to roles] option does not suit.
...
For more information about customizing `SecurityIdentity`, see the link:{quarkusio-guides}/security-customization#security-identity-customization[Security identity customization] section in the Quarkus "Security tips and tricks" guide.
...
For more information, see the link:{quarkusio-guides}/security-webauthn[Secure a Quarkus application by using the WebAuthn authentication mechanism]
endif::no-webauthn-authentication[]
...
|Bearer token authentication mechanism|link:{default-url}/openid-connect-authentication/security-oidc-bearer-token-authentication[OIDC Bearer token authentication]
|Authorization Code Flow authentication mechanism|link:{default-url}/openid-connect-authentication/security-oidc-code-flow-authentication[OpenID Connect (OIDC) Authorization Code Flow mechanism]
|OIDC and SAML Identity broker|link:{default-url}/openid-connect-authentication/security-oidc-code-flow-authentication#oidc-saml-broker[OpenID Connect (OIDC) Authorization Code Flow and SAML Identity broker]
|Multiple tenants that can support the Bearer token authentication or Authorization Code Flow mechanisms|link:{default-url}/openid-connect-authentication/security-openid-connect-multitenancy[Using OpenID Connect (OIDC) multi-tenancy]
|Securing Quarkus with commonly used OpenID Connect providers|link:{quarkusio-guides}/security-openid-connect-providers[Configuring well-known OpenID Connect providers]
|Using Keycloak to centralize authorization |link:{quarkusio-guides}/security-keycloak-authorization[Using OpenID Connect (OIDC) and Keycloak to centralize authorization]
ifndef::no-quarkus-keycloak-admin-client[]
|Configuring Keycloak programmatically |link:{quarkusio-guides}/security-keycloak-admin-client[Using the Keycloak admin client]
...
For more information about managing the individual tenant configurations in multitenant OIDC deployments, see the link:{default-url}/openid-connect-authentication/security-openid-connect-multitenancy#disable-tenant[Disabling tenant configurations] section in the "Using OpenID Connect (OIDC) multi-tenancy" guide.
====
...
For more information, see the link:{default-url}/openid-connect-authentication/security-openid-connect-client[OpenID Connect client and token propagation quickstart] and link:{default-url}/openid-connect-authentication/security-openid-connect-client-reference[OpenID Connect (OIDC) and OAuth2 client and filters reference] guides.
...
For more information, see the link:{quarkusio-guides}/security-jwt[Using JWT RBAC] guide.
...
For more information, see the Quarkus link:{quarkusio-guides}/security-oauth2[Using OAuth2] guide.
endif::no-quarkus-elytron-security-oauth2[]
...
For more information, see the Quarkus link:{default-url}/security-architecture/security-proactive-authentication[Proactive authentication] guide.

== References

* link:{default-url}/security-overview/security-overview[Quarkus Security overview]
* link:{default-url}/security-architecture/security-architecture[Quarkus Security architecture]
* link:{default-url}/security-architecture/security-identity-providers[Identity providers]
* link:{default-url}/authorization/security-authorize-web-endpoints-reference[Authorization of web endpoints]
