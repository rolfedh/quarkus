...
It introduces a policy enforcer that regulates access to secured resources.
Permissions defined in Keycloak govern access, offering flexible and dynamic Resource-Based Access Control (RBAC).
...
Instead of relying on explicit mechanisms like role-based access control (RBAC), `quarkus-keycloak-authorization` determines request permissions dynamically based on resource attributes such as name, identifier, or Uniform Resource Identifier (URI). This is achieved by sending a `quarkus-oidc`-verified bearer access token to Keycloak Authorization Services, which evaluates the token and makes an authorization decision.

Use `quarkus-keycloak-authorization` only if you are using Keycloak and have enabled Keycloak Authorization Services to handle authorization decisions.
Otherwise, use `quarkus-oidc` instead.

By shifting authorization responsibilities outside your application, you enhance security through diverse access control methods and reduce the need for frequent redeployments as security requirements evolve. Keycloak serves as a centralized authorization hub, efficiently managing protected resources and their corresponding permissions.

For more information, see the xref:security-oidc-bearer-token-authentication.adoc[OIDC Bearer Token Authentication] guide.
The Bearer Token authentication mechanism performs authentication and creates a security identity. The `quarkus-keycloak-authorization` extension then applies a Keycloak authorization policy to this identity based on the current request path and other policy settings.
