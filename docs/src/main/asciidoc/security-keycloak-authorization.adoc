...
- Keycloak Authorization Services is enabled in your environment to handle authorization decisions.

For other cases, consider using the simpler `quarkus-oidc` extension instead.
...
3. Allows Keycloak to evaluate resource-based permissions dynamically, using attributes such as resource name, identifier, or URI.
...
This extension is compatible only with Quarkus xref:security-oidc-bearer-token-authentication.adoc[OIDC service applications]. It replaces explicit mechanisms such as role-based access control with dynamic authorization policies.
...
Here's the improved and optimized version of the section with a focus on clarity, structure, and adherence to best practices:
== Architecture
...
Access to these endpoints is controlled using bearer tokens. To gain access, the following conditions must be met:
...
This extension implements a Keycloak Adapter for Quarkus applications and offers all the necessary capabilities to integrate with a Keycloak server and perform bearer token authorization.
