...
:context: security-oidc-code-flow-authentication
...
See also the link:{default-url}/openid-connect-authentication/security-oidc-configuration-properties-reference[OIDC configuration properties] reference guide.

To learn about how you can protect web applications by using the OIDC Authorization Code Flow mechanism, see link:{default-url}/openid-connect-authentication/security-oidc-code-flow-authentication-tutorial[Protect a web application by using OIDC authorization code flow].

If you want to protect service applications by using OIDC Bearer token authentication, see link:{default-url}/openid-connect-authentication/security-oidc-bearer-token-authentication[OIDC Bearer token authentication].

For information about how to support multiple tenants, see link:{default-url}/openid-connect-authentication/security-openid-connect-multitenancy[Using OpenID Connect Multi-Tenancy].
...
[id="oidc-provider-client-authentication"]
...
The following example shows the secret retrieved from a link:{quarkusio-guides}/credentials-provider[credentials provider]:
...
.Example of `client_secret_jwt`, where the secret is retrieved from a link:{quarkusio-guides}/credentials-provider[credentials provider]:
...
Some providers, such as the link:{quarkusio-guides}/security-openid-connect-providers#strava[Strava OAuth2 provider], require client credentials be posted as HTTP POST query parameters:
...
[id="oidc-request-filters_{context}"]
...
[id="access_id_and_access_tokens"]
...
[id="token-claims-roles"]
==== Mapping token claims and `SecurityIdentity` roles

The way the roles are mapped to the SecurityIdentity roles from the verified tokens is identical to how it is done for the link:{default-url}/openid-connect-authentication/security-oidc-bearer-token-authentication[Bearer tokens].
...
For more information, see link:{quarkusio-guides}/security-customization#security-identity-customization[SecurityIdentity customization].
You can also map `SecurityIdentity` roles created from token claims to deployment-specific roles with the link:{default-url}/authorization/security-authorize-web-endpoints-reference#map-security-identity-roles[HTTP Security policy].
...
[id="token-verification-introspection_{context}"]
...
For more information, see the link:{default-url}/openid-connect-authentication/security-oidc-bearer-token-authentication#token-verification-introspection[Token verification and introspection] section of the "Quarkus OpenID Connect (OIDC) Bearer token authentication" guide.
...
[id="token-introspection-userinfo-cache_{context}"]
...
For more information about using the default token cache or registering a custom cache implementation, see link:{default-url}/openid-connect-authentication/security-oidc-bearer-token-authentication#token-introspection-userinfo-cache[Token introspection and UserInfo cache].

[id="jwt-claim-verification_{context}"]
==== JSON web token claim verification

For information about the claim verification, including the `iss` (issuer) claim, see the link:{default-url}/openid-connect-authentication/security-oidc-bearer-token-authentication#jwt-claim-verification[JSON Web Token claim verification] section.
...
Depending on your database, include and configure exactly one link:{quarkusio-guides}/reactive-sql-clients[Reactive SQL client].
...
For more information, refer to the link:{quarkusio-guides}/hibernate-orm[Hibernate ORM] guide.
...
Note that a cookie name varies when using link:{default-url}/openid-connect-authentication/security-openid-connect-multitenancy[OpenID Connect Multi-Tenancy].
...
[TIP]
====
If you have a link:{default-url}/openid-connect-authentication/security-oidc-bearer-token-authentication#single-page-applications[single page application for service applications] where your OIDC provider script such as `keycloak.js` is managing an authorization code flow, then that script will also control the SPA authentication session lifespan.
====
...
This approach is similar to the OIDC xref:user-info_{context}[UserInfo] approach, with `UserInfo` fetched by Quarkus OIDC on your behalf.
...
, which you can extend further as described in the xref:session-management[session management] section.
...
If no introspection endpoint is present, you can attempt instead to request xref:user-info_{context}[UserInfo] from the provider as it will at least validate the access token.
...
[NOTE]
====
Requiring xref:user-info_{context}[UserInfo] involves making a remote call on every request.
...
For more information, see the link:{default-url}/openid-connect-authentication/security-oidc-bearer-token-authentication#token-introspection-userinfo-cache[Token Introspection and UserInfo cache] section of the "OpenID Connect (OIDC) Bearer token authentication" guide.
...
For more information about configuring other well-known providers, see link:{quarkusio-guides}/security-openid-connect-providers[OpenID Connect providers].
...
[id="listen-to-authentication-events"]
...
TIP: You can listen to other security events as described in the link:{quarkusio-guides}/security-customization#observe-security-events[Observe security events] section of the Security Tips and Tricks guide.

=== Propagating tokens to downstream services

For information about Authorization Code Flow access token propagation to downstream services, see the link:{default-url}/openid-connect-authentication/security-openid-connect-client-reference#token-propagation[Token Propagation] section.
...
You can check if implementing single-page applications (SPAs) the way it is suggested in the link:{default-url}/openid-connect-authentication/security-oidc-bearer-token-authentication#single-page-applications[Single-page applications] section of the "OpenID Connect (OIDC) Bearer token authentication" guide meets your requirements.
...
For more information, see the link:{quarkusio-guides}/security-cors#cors-filter[CORS filter] section of the "Cross-origin resource sharing" guide.
...
For more information, see the link:{quarkusio-guides}/http-reference#reverse-proxy[Running behind a reverse proxy] Vert.x documentation section.
...
[id="integration-testing_{context}"]
== Testing
...
[id="integration-testing-wiremock_{context}"]
=== Wiremock
...
[id="integration-testing-keycloak-devservices_{context}"]
=== Dev Services for Keycloak

Using link:{quarkusio-guides}/security-openid-connect-dev-services[Dev Services for Keycloak] is recommended for integration testing against Keycloak.
...
Finally, write a test code the same way as it is described in the xref:integration-testing-wiremock_{context}[Wiremock] section.
...
ifndef::no-deprecated-test-resource[]
[id="integration-testing-keycloak_{context}"]
...
If you need to do the integration testing against Keycloak then you are encouraged to do it with xref:integration-testing-keycloak-devservices_{context}[Dev Services for Keycloak].
...
Now, set the configuration and write the test code the same way as it is described in the xref:integration-testing-wiremock_{context}[Wiremock] section.
...
[id="integration-testing-security-annotation_{context}"]
...
For more information, see link:{default-url}/openid-connect-authentication/security-oidc-bearer-token-authentication#integration-testing-security-annotation[Use TestingSecurity with injected JsonWebToken].
...
* link:{default-url}/openid-connect-authentication/security-oidc-configuration-properties-reference[OIDC configuration properties]
* link:{quarkusio-guides}/security-openid-connect-providers[Configuring well-known OpenID Connect providers]
...
* link:{quarkusio-guides}/security-openid-connect-dev-services[Dev Services for Keycloak]
