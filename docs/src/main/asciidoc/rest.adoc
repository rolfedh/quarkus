...
This configuration improves performance, reduces memory overhead, and enhances efficiency in native applications.

This feature is available as a Technology Preview and is disabled by default.
To enable it, set the following configuration property:
...
When enabled, {project-name} generates `StdSerializer` implementations at build time for each class that requires JSON conversion.
The application then uses these generated serializers to serialize and deserialize objects returned by REST endpoints without relying on reflection.
